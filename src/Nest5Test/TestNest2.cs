...
namespace Nest5Test
{
    ...
    public class TestNest2
    {
        ...
        {
            //新增字段
            var result = _client.Map<TestModel2>(m => m.Index(indexName).Properties(p => p.String(s => s.Name("NewField").Index(FieldIndexOption.NotAnalyzed))));
        }
        ...
        public void DeleteByQuery(string indexName)
        {
            ...
            _client.DeleteByQuery<TestModel2>(
                //indexName,
                //typeof(TestModel2),
                s => s
                    .Index(indexName)
        ...
        public void UpdateByWhere(string indexName)
        {
            ...
            //todo:2.x 需要安装插件 前面两个参数必须传递
            _client.UpdateByQuery<TestModel2>(
                //indexName,
                //typeof(TestModel2),
                s =>s
                //.Index(indexName)
